...
Bot: ⚠️  @opted-out-user hasn't opted in to receive direct messages.

     Options:
     • Cancel - Don't create this reminder
     • Create without DM - Reminder will only post in server
     • Notify User - Ask them to opt-in first

     What would you like to do? (React with ❌ Cancel, 📋 Create, or 📢 Notify)
```
...
- Display warning with **interactive buttons** if user hasn't opted in:
  - **❌ Cancel** - Abort reminder creation
  - **📋 Create** - Create reminder with server-only notifications
  - **📢 Notify** - Send opt-in request to recipient
...
1. ✅ All opt-in/out operations are idempotent
2. ✅ Users can manage their communication preferences
3. ✅ Senders are clearly informed about opt-in status
4. ✅ No unwanted DMs sent to opted-out users
5. ✅ 100% test coverage for new functionality
6. ✅ 0 errors, minimal warnings from linter
