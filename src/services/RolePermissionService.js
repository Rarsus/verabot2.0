...
class RolePermissionService {
   ...
   * Checks Discord roles and bot owner list
  ...
  async getUserTier(userId, guildId, client) {
    if (!this.roleConfig.enabled) return 1;
   ...
   * Considers both permission and visibility settings
   * @param {string} userId - Discord user ID
   * @param {string} guildId - Discord guild ID
   * @param {string} commandName - Command name
   * @param {Object} client - Discord.js client
  ...
  /**
   * Get guild role mappings from database
   * @param {_guildId} _guildId - Guild ID (unused, reserved for future DB access)
  ...
  /**
   * Clear cache entry (when user roles change)
