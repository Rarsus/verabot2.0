...
function getDatabase() {
  if (!db) {
    ...
    // Ensure schema is set up on first initialization
    if (!schemaInitialized) {
      // Call setupSchema synchronously - it uses db.serialize() internally
      db.serialize(() => {
        // Create quotes table
        db.run(
          `
          ...
          (err) => {
            if (err) {
              logError('database.getDatabase.createQuotesTable', err, ERROR_LEVELS.CRITICAL);
            }
          }
        );
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              logError('database.getDatabase.createTagsTable', err, ERROR_LEVELS.CRITICAL);
            }
          }
        );
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              logError('database.getDatabase.createQuoteTagsTable', err, ERROR_LEVELS.CRITICAL);
            }
          }
        );
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              logError('database.getDatabase.createRatingsTable', err, ERROR_LEVELS.CRITICAL);
            }
          }
        );
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              logError('database.getDatabase.createSchemaVersions', err, ERROR_LEVELS.CRITICAL);
            }
          }
        );
