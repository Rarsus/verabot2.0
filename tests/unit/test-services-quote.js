...
(async () => {
  try {
    ...
    await db.addQuote('Test quote for getAllQuotes', 'Test Author');
    const quotes = await getAllQuotes();
    if (quotes.length > 0) {
...
(async () => {
  try {
    ...
    await db.addQuote('Unique search test phrase', 'Search Author');
    const results = await searchQuotes('Unique search test');
...
(async () => {
  try {
    ...
    await db.addQuote('Another test quote', 'UniqueAuthorName');
    const results = await searchQuotes('UniqueAuthorName');
...
(async () => {
  try {
    const results = await searchQuotes('xyznonexistentquery123456');
...
(async () => {
  try {
    ...
    await db.addQuote('CaseSensitiveTest quote', 'CaseAuthor');
    const results = await searchQuotes('casesensitive');
    if (results.length > 0) {
...
(async () => {
  try {
    await db.addQuote('This is a partial match test', 'Partial Author');
    const results = await searchQuotes('partial');
    if (results.length > 0) {
...
(async () => {
  try {
    ...
    if (Array.isArray(results)) {
      console.log('✅ Test 12 Passed: Search handles empty string');
      passed++;
    } else {
      console.error('❌ Test 12 Failed: Empty string search failed');
      failed++;
    }
  } catch (err) {
